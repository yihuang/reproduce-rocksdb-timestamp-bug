...
func compareTS(bz1 []byte, bz2 []byte) int {
	ts1 := binary.BigEndian.Uint64(bz1)
	ts2 := binary.BigEndian.Uint64(bz2)
	switch {
	case ts1 < ts2:
		return -1
	case ts1 > ts2:
...
func OpenDB(dir string) (*grocksdb.DB, *grocksdb.ColumnFamilyHandle, error) {
	opts := grocksdb.NewDefaultOptions()
	opts.SetCreateIfMissing(true)
	opts.SetComparator(CreateTSComparator())
	db, cfHandles, err := grocksdb.OpenDbColumnFamilies(opts, dir, []string{"default"},
		[]*grocksdb.Options{opts},
	)
	if err != nil {
		return nil, nil, err
	}

	return db, cfHandles[0], nil
}
